...
class Rose(Plants):
    states = {0: 'отсутствует', 1: 'росток', 2: 'ростет', 3: 'распустился', 4: 'опадает',
              5: 'сгнила', }
    ills = {0: 'появился вредитель - розанная тля', 1: 'появилась болезнь - мучнистая роса'}
    ...
    def __init__(self) -> None:
        ...
        self.state = 0
        self.weather_cond = 0
        self.limit_weather_cond_w = 0.7
        self.limit_weather_cond_d = -0.7
        self.ills = None
    ...
    def ill(self) -> None:
        print(f"У розы {Rose.ills[self.ills]}")
        write_in_file(f"У розы {Rose.ills[self.ills]}")
        self.state -= 0.2
    ...
    def print_state(self) -> None:
        Rose.change_state(self)
        print(f'Роза сейчас {Rose.states[self.state_grow]}')
        write_in_file(f'Роза сейчас {Rose.states[self.state_grow]}')
