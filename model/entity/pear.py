...
class Pear(Plants):
    states = {0: 'отсутствует', 1: 'саженец', 2: 'рост', 3: 'набухание почек', 4: 'распускание почек',
              5: 'цветение', 6: 'созревание', 7: 'созрела'}
    ...
    def __init__(self) -> None:
        ...
        self.state = 0
        self.weather_cond = 0
        self.limit_weather_cond_w = 15
        self.limit_weather_cond_d = -15
        self.ills = None
    ...
    def ill(self) -> None:
        print("У груши {}".format(Pear.ills[self.ills]))
        write_in_file("У груши {}".format(Pear.ills[self.ills]))
        self.state -= 0.2
    ...
    def print_state(self) -> None:
        Pear.change_state(self)
        print(f'Груша сейчас {Pear.states[self.state_grow]}')
        write_in_file(f'Груша сейчас {Pear.states[self.state_grow]}')

    def max_harvest(self) -> int:
        harvest = random.randint(15, 25)
        return harvest
